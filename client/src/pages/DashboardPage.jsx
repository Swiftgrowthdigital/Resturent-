...
function DashboardPageContent() {
  const { restaurantName, settings } = useMenu();
  ...
  const paginatedRows = useMemo(() => tabRows.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE), [tabRows, currentPage]);
  const updateQuery = useCallback((nextTab, nextPage, replace = false) => {
    const next = new URLSearchParams(searchParams);
    if (nextTab === 'Dashboard') next.delete('tab'); else next.set('tab', nextTab);
    if (nextPage <= 1) next.delete('page'); else next.set('page', String(nextPage));
    setSearchParams(next, { replace, preventScrollReset: true });
  }, [searchParams, setSearchParams]);
  useEffect(() => {
    if (tab !== 'Dashboard' && requestedPage !== currentPage) updateQuery(tab, currentPage, true);
  }, [currentPage, requestedPage, tab, updateQuery]);
  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6"><header className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-brand-300">{restaurantName} admin</p><h1 className="mt-1 text-3xl font-black text-white">Today's Orders</h1></div><span className={`premium-chip ${liveStatus === 'connected' ? 'text-emerald-300' : 'text-amber-300'}`}><span className={`h-2 w-2 rounded-full ${liveStatus === 'connected' ? 'bg-emerald-400 shadow-[0_0_12px_rgba(74,222,128,0.9)]' : 'animate-pulse bg-amber-400'}`} />{liveStatus === 'connected' ? 'Live Updates' : 'Reconnecting...'}</span></header><nav className="no-scrollbar mb-5 flex gap-2 overflow-x-auto border-b border-white/10 pb-3">{tabs.map((item) => <button key={item} onClick={() => updateQuery(item, 1)} className={`focus-ring whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold transition duration-200 ${tab === item ? 'bg-orange-500 text-white' : 'bg-white/[.05] text-slate-300 hover:bg-white/[.1]'}`}>{item}</button>)}</nav>
    ...
    {tab === 'Menu' ? <Management title="Manage Menu" action="Add Item" onAdd={() => open('food')} rows={paginatedRows} totalItems={foods.length} currentPage={currentPage} totalPages={totalPages} onPageChange={(page) => updateQuery(tab, page)} empty="No menu items yet." render={(f) => <Row image={f.image} name={f.name} detail={`${f.category?.name || 'Uncategorized'} · ${formatCurrency(f.price, currency)}`} state={f.available !== false && !f.outOfStock ? 'Enabled' : 'Disabled'} onEdit={() => open('food', f)} onDelete={() => remove('food', f)} />} /> : null}
    {tab === 'Seats' ? <Management title="Seat Management" action="Add Seat" count={`${seats.length} total seats`} onAdd={() => open('seat')} rows={paginatedRows} totalItems={seats.length} currentPage={currentPage} totalPages={totalPages} onPageChange={(page) => updateQuery(tab, page)} empty="No seats configured." render={(s) => <Row name={s.seatNumber} state={s.active !== false ? 'Enabled' : 'Disabled'} onEdit={() => open('seat', s)} onDelete={() => remove('seat', s)} />} /> : null}
...
export function DashboardPage() {
  ...
  async function signIn(event) {
    ...
    try {
      ...
      if (!data.token) throw new Error('Login did not return an authentication token.');
  ...
  return <main className="mx-auto flex min-h-screen max-w-md items-center px-4"><form onSubmit={signIn} className="premium-card w-full p-6"><p className="text-xs font-bold uppercase tracking-[.2em] text-brand-300">{restaurantName} admin</p><h1 className="mt-2 text-3xl font-black text-white">Dashboard sign in</h1><p className="mt-2 text-sm text-slate-400">Use the dashboard password configured on the server.</p><label className="mt-6 block text-sm font-semibold text-slate-200">Password<input className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-white outline-none focus:border-orange-400" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label><Button className="mt-5 w-full" type="submit" loading={submitting}>Sign in</Button></form></main>;
}
function Dashboard({ stats, currency }) { const cards = [['Total Orders Today', stats.total], ['Pending Orders', stats.pending], ['Confirmed Orders', stats.confirmed], ['Cancelled Orders', stats.cancelled], ["Today's Revenue", formatCurrency(stats.revenue, currency)]]; return <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{cards.map(([label, value]) => <article key={label} className="premium-card p-4"><p className="text-xs font-semibold text-slate-400">{label}</p><p className="mt-2 text-2xl font-black text-white">{value}</p></article>)}</section>; }
function Orders({ orders, totalItems, currentPage, totalPages, onPageChange, onAction }) { const { settings } = useMenu(); return <section className="space-y-3">{orders.length ? <><div className="space-y-3">{orders.map((o) => <article key={o._id} className="premium-card p-4"><div className="flex flex-wrap justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-black text-white">{o.orderNumber}</h2><StatusPill status={o.status} /></div><p className="mt-1 text-sm text-slate-400">Seat {o.seatNumber} · {new Date(o.createdAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</p></div><p className="font-black text-white">{formatCurrency(o.grandTotal ?? o.totalAmount, settings?.currency)}</p></div><div className="mt-3 rounded-xl bg-white/[.04] p-3 text-sm text-slate-200">{toArray(o.items).map((i) => <p key={i.name}>{i.quantity} × {i.name}</p>)}</div>{o.status === 'Pending' ? <div className="mt-3 flex gap-2"><Button size="sm" onClick={() => onAction(o._id, 'confirm')} leftIcon={Check}>Confirm Order</Button><Button size="sm" variant="secondary" onClick={() => onAction(o._id, 'cancel')} leftIcon={X}>Cancel Order</Button></div> : null}</article>)}</div><div className="premium-card overflow-hidden"><Pagination currentPage={currentPage} totalPages={totalPages} totalItems={totalItems} pageSize={PAGE_SIZE} onPageChange={onPageChange} /></div></> : <EmptyState title="No orders today" description="Today's customer orders appear here." icon={PackageSearch} />}</section>; }
function Management({ title, action, count, onAdd, rows, totalItems, currentPage, totalPages, onPageChange, empty, render }) { return <section className="premium-card overflow-hidden"><div className="flex items-center justify-between gap-3 p-5"><div><h2 className="text-2xl font-black text-white">{title}</h2>{count ? <p className="mt-1 text-sm text-slate-400">{count}</p> : null}</div><Button onClick={onAdd} leftIcon={Plus}>{action}</Button></div><div className="divide-y divide-white/10 border-t border-white/10">{rows.length ? rows.map(render) : <p className="p-6 text-center text-sm text-slate-400">{empty}</p>}</div><Pagination currentPage={currentPage} totalPages={totalPages} totalItems={totalItems} pageSize={PAGE_SIZE} onPageChange={onPageChange} /></section>; }
function Row({ image, name, detail, state, onEdit, onDelete }) { return <div className="flex items-center justify-between gap-3 p-4"><div className="flex min-w-0 items-center gap-3">{image ? <img src={image} alt="" className="h-12 w-12 rounded-lg object-cover" /> : null}<div><p className="font-bold text-white">{name}</p>{detail ? <p className="text-sm text-slate-400">{detail}</p> : null}<p className={`text-xs ${state === 'Enabled' || state === 'Active' ? 'text-emerald-300' : 'text-rose-300'}`}>{state}</p></div></div><div className="flex gap-1"><Button size="sm" variant="ghost" onClick={onEdit} aria-label="Edit"><Pencil className="h-4 w-4" /></Button><Button size="sm" variant="ghost" className="text-rose-300" onClick={onDelete} aria-label="Delete"><Trash2 className="h-4 w-4" /></Button></div></div>; }
function Modal({ modal, form, categories, saving, onChange, onClose, onSubmit }) { const type = modal.type; const title = `${modal.entity ? 'Edit' : 'Add'} ${type === 'food' ? 'Menu Item' : type === 'seat' ? 'Seat' : 'Category'}`; return <div className="fixed inset-0 z-50 flex items-end bg-slate-950/70 p-4 backdrop-blur-sm sm:items-center sm:justify-center"><form onSubmit={onSubmit} className="premium-card w-full max-w-md p-5"><div className="flex justify-between"><h2 className="text-xl font-black text-white">{title}</h2><button type="button" onClick={onClose}><X className="text-white" /></button></div><div className="mt-5 grid gap-4">{type === 'seat' ? <Input label="Seat Number" value={form.seatNumber || ''} onChange={(e) => onChange({ seatNumber: e.target.value })} required /> : <Input label={type === 'food' ? 'Item Name' : 'Category Name'} value={form.name || ''} onChange={(e) => onChange({ name: e.target.value })} required />}{type === 'category' ? <Input label="Icon (optional)" value={form.icon || ''} onChange={(e) => onChange({ icon: e.target.value })} /> : null}{type === 'food' ? <><label className="text-sm text-slate-200">Category<select className="mt-2 w-full rounded-xl bg-slate-900 p-3" value={form.category || ''} onChange={(e) => onChange({ category: e.target.value })}>{categories.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}</select></label><Input label="Price" type="number" min="0" value={form.price || ''} onChange={(e) => onChange({ price: e.target.value })} required /></> : null}{type !== 'seat' ? <label className="text-sm text-slate-200">Image (JPEG, PNG, or WebP; max 5MB)<input className="mt-2 block w-full text-sm text-slate-300" type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => onChange({ image: e.target.files?.[0] || null })} /></label> : null}<label className="flex justify-between rounded-xl bg-white/[.05] p-3 text-sm text-white"><span>Enabled</span><input type="checkbox" checked={type === 'category' ? form.status !== false : type === 'seat' ? form.active !== false : form.available !== false} onChange={(e) => onChange(type === 'category' ? { status: e.target.checked } : type === 'seat' ? { active: e.target.checked } : { available: e.target.checked })} /></label></div><div className="mt-5 flex justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit" loading={saving}>Save</Button></div></form></div>; }
