...
function DashboardPageContent() {
  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6"><header className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-brand-300">{restaurantName} admin</p><h1 className="mt-1 text-3xl font-black text-white">Today's Orders</h1></div><span className="premium-chip">Live updates on</span></header><nav className="no-scrollbar mb-5 flex gap-2 overflow-x-auto border-b border-white/10 pb-3">{tabs.map((item) => <button key={item} onClick={() => setTab(item)} className={`whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold transition ${tab === item ? 'bg-orange-500 text-white' : 'bg-white/[.05] text-slate-300 hover:bg-white/[.1]'}`}>{item}</button>)}</nav>
    {tab === 'Dashboard' ? <Dashboard stats={stats} currency={currency} /> : null}
    {tab === 'Orders' ? <Orders orders={orders} onAction={orderAction} /> : null}
    {tab === 'Categories' ? <Management title="Categories" action="Create Category" onAdd={() => open('category')} rows={categories} empty="No categories yet." render={(c) => <Row image={c.image} name={`${c.icon || ''} ${c.name}`} state={c.status !== false ? 'Active' : 'Disabled'} onEdit={() => open('category', c)} onDelete={() => remove('category', c)} />} /> : null}
    {tab === 'Menu' ? <Management title="Manage Menu" action="Add Item" onAdd={() => open('food')} rows={foods} empty="No menu items yet." render={(f) => <Row image={f.image} name={f.name} detail={`${f.category?.name || 'Uncategorized'} · ${formatCurrency(f.price, currency)}`} state={f.available !== false && !f.outOfStock ? 'Enabled' : 'Disabled'} onEdit={() => open('food', f)} onDelete={() => remove('food', f)} />} /> : null}
